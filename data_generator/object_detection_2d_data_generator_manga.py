    ...
    def parse_manga(self,
        ...
        for images_dir, image_set_filename, annotations_dir in zip(images_dirs, image_set_filenames, annotations_dirs):
            ...
            with open(image_set_filename) as f:
                manga_ids = [line.strip() for line in f] # Note: These are strings, not integers.
                self.manga_ids += manga_ids


        # manga_id means mange_id
        for manga_id in manga_ids:
            for name in os.listdir(images_dir+manga_id):
                page = int(name[:-4])
                filename = '{}'.format(manga_id) +'/'+ name
                

                folder = manga_id # In case we want to return the folder in addition to the image file name. Relevant for determining which dataset an image belongs to.
                #filename = soup.filename.text

                boxes = [] # We'll store all boxes for this image here
                try:
                    objects = p.annotations[manga_id]["book"]["pages"]["page"][page]['text'] # Get a list of all objects in this image
                except:
                    continue
        ...
        if ret:
            return self.filenames, self.labels, self.manga_ids
